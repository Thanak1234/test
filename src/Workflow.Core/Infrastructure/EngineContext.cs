        ...
        public static IEngine CreateEngineInstance()
        {
			...
			if (engineTypeSetting.HasValue())
            {
                ...
                if (!typeof(IEngine).IsAssignableFrom(engineType))
					throw new ConfigurationErrorsException("The type '" + engineType + "' doesn't implement 'Workflow.Enterprise.Core.Infrastructure.IEngine' and cannot be configured in /configuration/appSettings/add[@key=sm:EngineType] for that purpose.");
