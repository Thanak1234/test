...
namespace Workflow
{
    public static class DataAccessExtensions
    {
        ...
        public static IEnumerable<object> GetResult(IDataReader reader) {
            List<object> values = new List<object>();
            while (reader.Read()) {
                var dataItem = new ExpandoObject();
                IDictionary<string, object> content = (IDictionary<string, object>)dataItem;
                for (int i = 0; i < reader.FieldCount; i++) {
                    string name = reader.GetName(i);
                    name = NameHelper.TransformName(name);
                    name = NameHelper.CleanName(name);
