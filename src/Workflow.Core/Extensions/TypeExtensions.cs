...
namespace Workflow
{
    ...
    public static class TypeExtensions
    {
        ...
        /* build hierachy level */
        #region Convert List to Hierachy Level Type
        public static IEnumerable<object> GetHierarchy<T>(this IEnumerable<T> obj) where T : Hierarchy
        {
            return GetHierarchy(obj, obj.ToList());
        }

        public static IEnumerable<object> GetHierarchyLevel<T>(this IEnumerable<T> obj, int level = 1) where T : Hierarchy
        {
            return GetHierarchy(obj, obj.ToList(), level);
        }

        public static IEnumerable<object> GetHierarchy<T>(this object obj, int parentid, IList<T> parent) where T : Hierarchy
        {
            var models = new List<object>();
            foreach (var child in parent.Where(t => t.ParentId == parentid))
            {
                var children = GetHierarchy(obj, child.Id, parent).ToList();
                if (children.Count > 0)
                {
        ...
        public static IEnumerable<object> GetHierarchy<T>(this object obj, int parentid, IList<T> parent, int level) where T : Hierarchy
        {
            var models = new List<object>();
            foreach (var child in parent.Where(t => (parentid == -1 || t.ParentId == parentid) && t.Level == level))
            {
                var children = GetHierarchy(obj, child.Id, parent, (child.Level + 1)).ToList();
                if (children.Count > 0)
                {
                    child.Children = children;
                }
        ...
        // Root Level
        public static IEnumerable<object> GetHierarchy<T>(this object obj, IList<T> parent, int level) where T : Hierarchy
        {
            return GetHierarchy(obj, (level > 1?-1:0), parent, level);
        }

        public static IEnumerable<object> GetHierarchy<T>(this object obj, IList<T> parent) where T : Hierarchy
        {
            return GetHierarchy(obj, 0, parent);
        }
        #endregion
    }
